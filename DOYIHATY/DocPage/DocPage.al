page 50112 "Document Page by DOYIHATY"
{
    ...
    begin
        ...
        LastDay := CalcDate('<CM>');
        LastDayTime := CreateDateTime(LastDay, Time);

        if (Rec."Drop-off Date" = LastDayTime) OR (Rec."Pick-up Date" = LastDayTime)
        then begin
            Message('It isn''t possible to rent or return transport on the last day of the month, please try again!');
        end;
