table 50113 "Rent Order Table"
{
    ...
    keys
    {
        ...
        key(QuaternaryKey; "Phone Number")
        {
        }
        key(TK; Status)
        {
    ...
    begin
        CarT.Reset();
        CarT.SetRange(Vehicle, "Desired Car");

        if CarT.FindFirst() AND (Status = 'Open') then begin
            CarT."Car Status" := 'In Use';
            CarT.Modify(true);
        end else begin
            CarT."Car Status" := 'Not In Use';
            CarT.Modify(true);
        end;
    end;
}